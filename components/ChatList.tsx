...
const ChatList: React.FC<Props> = ({ chats, searchQuery, activeTab, archivedChats, onChatClick, onPin, onArchive, onDelete, onTabChange }) => {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('server-update', loadExtras);
  }, []);
  ...
  const handleAdClick = () => {
    if(activeAd?.link) window.open(activeAd.link, '_blank');
  };
  ...
  return (
    <div 
      ...
      onScroll={(e) => {
        ...
        // Only allow pull-to-archive on main tab
    ...
    >
      ...
      <div className="p-4 space-y-4 pb-24">
        ...
        {activeAd && activeAd.isActive && !searchQuery && activeTab === 'All' && (
            <div onClick={handleAdClick} className="relative flex items-center p-3 gap-4 bg-gradient-to-r from-[#1c1c1e] to-[#252527] rounded-[24px] shadow-lg border border-blue-500/20 cursor-pointer animate-fade-in mb-2 group">
                {activeAd.image && (
