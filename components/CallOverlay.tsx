...
const CallOverlay: React.FC<CallProps> = ({ user, callData, onClose }) => {
  ...
  // If I am caller, I want to see Receiver. If I am receiver, I want to see Caller.
  const peerName = isCaller ? "User" : callData.callerName; // We simplified peer name retrieval for this component
  const peerAvatar = isCaller ? "" : callData.callerAvatar;

  // 1. Sync Call Status with DB
  useEffect(() => {
    const checkStatus = async () => {
        ...
        if (!myCall) {
            handleCleanup(); // Call removed/ended
  ...
  useEffect(() => {
    const startMedia = async () => {
      try {
        ...
        // Continue without media if denied, or show error
      }
    };
    
    // Start media immediately for preview
    startMedia();
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-[#0f1115] text-white flex flex-col font-sans animate-fade-in overflow-hidden">
        ...
        {/* VIDEO BACKGROUND (Simulated P2P: Showing Self as Remote for Demo or Self Preview) */}
        {callData.type === 'video' && (
        ...
        {/* AUDIO BACKGROUND (Blurred Avatar) */}
        {callData.type === 'audio' && (
        ...
        <div className="relative z-10 pt-16 flex flex-col items-center">
            {callData.type === 'audio' && (
                 <div className="w-32 h-32 rounded-full border-4 border-white/10 shadow-2xl mb-6 overflow-hidden bg-gray-800">
                    <img src={peerAvatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${callData.callerId}`} className="w-full h-full object-cover" />
                 </div>
            )}
            
            <h2 className="text-3xl font-black mb-2 drop-shadow-md">{isCaller ? (callData as any).receiverId : callData.callerName}</h2>
